...
app.get("/t/:id", async (req, res) => {
  if (req.session.user) {
    try {
      const currentTweet = await NLP.findOne({
        id: req.params.id,
        user: req.session.user,
      });
      res.render("annotate", {
...
app.post("/t/:id", async (req, res) => {
  ...
  const updated = await NLP.findOneAndUpdate(
...
app.post("/", (req, res) => {
  if (
    req.body.user === "henry" ||
    req.body.user === "mike" ||
    req.body.user === "samantha" ||
    req.body.user === "yulong"
  ) {
...
app.get("/initialize/:offset", (req, res) => {
  // tools only for these four users
  saveTweets(
    ["henry", "mike", "samantha", "yulong"],
    parseInt(req.params.offset)
  ).then(res.send("Saved Successful!"));
});
...
async function saveProgress(currentProgress, user) {
  try {
    const savedProgress = await Progress.findOneAndUpdate(
      { user: user },
      { $set: { lastAnnotation: currentProgress } }
    );
    console.log("Progress saved.");
  } catch (err) {
    console.log(err);
...
app.get("/api/getTweets/:hashtag/:amount", (req, res) => {
